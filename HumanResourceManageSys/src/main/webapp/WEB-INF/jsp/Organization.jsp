...
<div id="tt" class="easyui-tabs" style="width:100%;height:100%;">  
    ...
    <form>
    	...
    	<br/>
    		...
    		<input type="button" onclick="addFirstKind()" value="添加" style="float: right;">
    		<br/><br/>
    ...
    <form>
    	<table id="salaryNameTable">
    		<tr>
    			<td>一级机构名称:</td>
    			<td><select type="text" class="firstKindIdSencond"  style="width: 150px;">
    		...
    		<tr>
    			<td>二级机构名称:</td>
    			<td>
    			<input type="text" class="secondKindfskId" value="" hidden="hidden"/>
    			<input type="text" class="secondKindName" value=""></td>
    		</tr>
    		<tr>
    			<td>二级机构薪酬发放责任人:</td>
    			<td><select type="text" class="secondKindSalaryId" style="width: 150px;" >
    				<option value="1001">薪酬经理</option>
    				<option value="1002">薪酬专员</option>
    			</select></td>
    		</tr>
    		<tr>
    			<td>二级机构销售责任人:</td>
    			<td><select type="text" class="secondKindSaleId"  style="width: 150px;">
    				<option value="10001">销售经理</option>
    				<option value="10002">销售专员</option>
    			</select></td>
    		</tr>
    	...
    	<br/>
    		<br/>
    		<input type="button" onclick="updateSecondKind()" value="确认修改" style="float: right;">
    		<input type="button" onclick="addSecondKind()" value="添加" style="float: right;">
    		<br/>
    		<br/>
     ...
     <div title="三级机构设置" style="padding:20px;display:none;"> 
    	...
    	<table id="salaryNameTable">
    	<tr>
    			<td>一级机构名称：</td><td><input style="width: 150px;" id="cc3" class="easyui-combobox"
			data-options=" 
		...
		<tr>
		<td>二级机构名称：</td><td><input style="width: 150px;" id="cc4" class="easyui-combobox"
    		...
    		<tr>
    			<td>三级机构名称:</td>
    			<td>
    			<input type="text" class="thirdKindftkId" value="" hidden="hidden"/>
    			<input type="text" class="thirdKindName" value=""></td>
    		</tr>
    		<tr>
    			<td>三级机构销售责任人:</td>
    			<td><select type="text" class="thirdKindSaleId" style="width: 150px;" >
    				<option value="1001">薪酬经理</option>
    				<option value="1002">薪酬专员</option>
    			</select></td>
    		</tr>
    		<tr>
    			<td>是否为零售店:</td>
    			<td><select type="text" class="thirdKindIsRetail"  style="width: 150px;">
    	...
    	<br/>
    		<br/>
    		<input type="button" onclick="updateThirdKind()" value="确认修改" style="float: right;">
    		<input type="button" onclick="addThirdKind()" value="添加" style="float: right;">
    		<br/>
    		<br/>
...
function addSecondKind(){//菜单添加  
	var firstKindName=$(".firstKindIdSencond").find("option:selected").text();
	...
	var secondKindSaleId=$('.secondKindSaleId').val();
		$.post('addSecondKind',{secondKindName:secondKindName,secondKindSalaryId:secondKindSalaryId,firstKindName:firstKindName,firstKindId:firstKindId,secondKindSaleId:secondKindSaleId},function(data){
			if(data==1){
				alert('添加成功');
				$('.secondKindName').val('');
				...
				alert('二级机构名称已存在，请勿重复添加！');var Mainmenu=$('.Mainmeu').val();
				$('.secondKindName').val('');
			}
		},'text');
}
...
function addThirdKind(){//菜单添加  
	...
	var thirdKindIsRetail=$('.thirdKindIsRetail').val();
		$.post('addThirdKind',{thirdKindIsRetail:thirdKindIsRetail,secondKindId:secondKindId,secondKindName:secondKindName,thirdKindName:thirdKindName,firstKindName:firstKindName,firstKindId:firstKindId,thirdKindSaleId:thirdKindSaleId},function(data){
			if(data==1){
				alert('添加成功');
				$('.thirdKindName').val('');
				...
				alert('三级机构名称已存在，请勿重复添加！');var Mainmenu=$('.Mainmeu').val();
				$('.thirdKindName').val('');
			}
		},'text');
}
...
////2.修改前显示 
function alertSecondKind(second_kind_name){
	$.post('findSecondKindByName',{second_kind_name:second_kind_name},function(data){
...
function alertThirdKind(first_kind_name){
	$.post('findFirstKindByName',{first_kind_name:first_kind_name},function(data){
		$('.firstKindffkId').val(data[0].ffk_id);
		$('.firstKindName').val(data[0].first_kind_name);
		$('.firstKindSalaryId').val(data[0].first_kind_salary_id);
...
function updateSecondKind(){
    		...
    		var secondKindSalaryId=$('.secondKindSalaryId').val();
    		var secondKindSaleId=$('.secondKindSaleId').val();   			
	$.post('updateSecondKind',{fskid:fskid,secondKindName:secondKindName,secondKindSalaryId:secondKindSalaryId,firstKindName:firstKindName,firstKindId:firstKindId,secondKindSaleId:secondKindSaleId},function(data){
		if(data==0){
			alert('修改失败')
		}else{
			alert('修改成功')
...
function deleteSecondKind(id){
	$.post('deleteSecondKind',{fsk_id:id},function(data){
		if(data==1){
			alert('删除成功');
			$('#showSecondKind').datagrid('reload');//刷新
		}else if(data=0){
...
function deleteThirdKind(id){
	$.post('deleteThirdKind',{ftk_id:id},function(data){
		if(data==1){
			alert('删除成功');
			$('#showThirdKind').datagrid('reload');//刷新
		}else if(data=0){
...
$('#showSecondKind').datagrid({
	...
	columns:[[
	          ...
	          {field:'cz',title:'操作',width:100,formatter:function(value,row,index){
	        	  return '<a href="javascript:deleteSecondKind('+row.fsk_id+')">删除</a> <a href="javascript:alertSecondKind(\''+row.second_kind_name+'\')">修改</a>';
	          }}
	     ]]
});
//3
$('#showThirdKind').datagrid({
	...
	columns:[[
				...
				{field:'cz',title:'操作',width:100,formatter:function(value,row,index){
		        	  return '<a href="javascript:deleteThirdKind('+row.ftk_id+')">删除</a> <a href="javascript:alertThirdKind(\''+row.third_kind_name+'\')">修改</a>';
		          }}
	     ]]
});
</script>
