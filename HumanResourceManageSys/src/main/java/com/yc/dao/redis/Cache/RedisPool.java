...
public class RedisPool {
	...
	private RedisPool() {
		...
		if (bundle == null) {
			throw new IllegalArgumentException("[redis.porperties] is not found");
		}
		...
		config.setTestOnReturn(Boolean.valueOf(bundle.getString("redies.pool.testOnReturn")));

		pool = new JedisPool(config, bundle.getString("redis.port"), Integer.valueOf(bundle.getString("redis.port")));
